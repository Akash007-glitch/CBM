...
import { X, Plus, User, Building, Phone, Mail, MapPin, DollarSign, Briefcase, FileText, CreditCard, CheckCircle2 } from "lucide-react";
...
// ── Add New Customer Modal (Exact Stitch Specification & Screenshot) ──
...
export const AddCustomerModal: React.FC<ModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const companyToUse = companyName.trim() || "Acme Corp";
    const nameToUse = contactPerson.trim() || companyToUse;
    const openBal = parseFloat(openingBalance) || 0;

    addCustomer({
      name: nameToUse,
      company: companyToUse,
      email: email.trim() || `${companyToUse.toLowerCase().replace(/[^a-z0-9]/g, "")}@company.com`,
      phone: phone.trim() || "(555) 000-0000",
      outstandingBalance: openBal,
      status: "Active",
    });

    setSuccessToast(`Customer "${companyToUse}" created successfully!`);
      ...
      <div className="bg-white w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-[#E2E8F0] animate-in fade-in zoom-in duration-200">
        ...
        <form id="add-customer-form" onSubmit={handleSubmit} className="p-6 overflow-y-auto flex flex-col gap-6 text-sm">
          ...
          <section className="flex flex-col gap-3">
            <h3 className="text-xs font-bold text-[#0F766E] uppercase tracking-widest">
              BASIC INFORMATION
            </h3>
          ...
          <section className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold text-[#3E4947]">ZIP</label>
                  <input
                    type="text"
                    value={zip}
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-[#3E4947]">Customer Category</label>
                <select
                  value={category}
...
// ── Multi-Tab Quick Add Action Modal ──────────────────────────────────────────

export const QuickAddModal: React.FC<ModalProps> = ({ isOpen, onClose }) => {
  const createInvoice = useDashboardStore((s) => s.createInvoice);
  ...
  // Form states for invoice/payment
  const [customerName, setCustomerName] = useState(customers[0]?.name ?? "Acme Corp");
  const [salesmanName, setSalesmanName] = useState(salesmen[0]?.name ?? "Raj Patel");
  ...
  const handleActionSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === "invoice") {
      ...
      setToastMsg(`Invoice generated for ₹${numAmount.toLocaleString("en-IN")}!`);
    } else if (activeTab === "payment") {
      recordPayment({
        customerName,
        amount: numAmount,
      });
      setToastMsg(`Payment of ₹${numAmount.toLocaleString("en-IN")} logged!`);
    }
        ...
        <div className="grid grid-cols-3 p-1 bg-[#EFF4FF] rounded-lg mt-4 text-xs font-bold">
          ...
          <button
            ...
            className={`py-1.5 rounded-md transition-all cursor-pointer ${
              activeTab === "invoice" ? "bg-white text-[#0B1C30] shadow-2xs" : "text-[#3E4947]"
            }`}
          >
            + Invoice
          </button>
          <button
            ...
            className={`py-1.5 rounded-md transition-all cursor-pointer ${
              activeTab === "payment" ? "bg-white text-[#0B1C30] shadow-2xs" : "text-[#3E4947]"
            }`}
          >
            + Payment
          </button>
...
// ── Sub-Pages Views for Sidebar Tabs ──────────────────────────────────────────
            ...
            {customers.map((c) => (
              <tr key={c.id} className="hover:bg-[#F8F9FF]">
                <td className="p-4 font-bold text-[#0B1C30]">
                  {c.name} <span className="text-[#3E4947] font-normal">({c.company})</span>
                </td>
                <td className="p-4 text-[#3E4947]">
                  {c.email}<br /><span className="text-xs text-[#6E7977]">{c.phone}</span>
                </td>
                <td className="p-4 font-semibold text-[#0B1C30]">₹{c.totalPurchases.toLocaleString("en-IN")}</td>
                <td className="p-4 font-bold text-[#BA1A1A]">₹{c.outstandingBalance.toLocaleString("en-IN")}</td>
                <td className="p-4">
                  <span className="px-2.5 py-1 rounded text-xs font-bold bg-[#0F766E]/10 text-[#0F766E]">
                    {c.status}
                  </span>
                </td>
              </tr>
  ...
  return (
    <div className="space-y-6 max-w-[1440px] mx-auto">
      <div>
        ...
        <p className="text-sm text-[#3E4947] font-medium">Real-time performance analytics</p>
      </div>
        ...
        {salesmen.map((s) => (
          <div key={s.id} className="bg-white p-6 rounded-[14px] border border-[#E2E8F0] shadow-xs flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[#6E7977]">{s.email}</p>
              <div className="flex items-center gap-4 mt-2 text-xs font-semibold text-[#3E4947]">
                ...
                <span>Deals: {s.deals}</span>
                <span className="text-amber-600">Rating: ★ {s.rating}</span>
              </div>
            </div>
          </div>
        ))}
    ...
    <div className="space-y-6 max-w-[1440px] mx-auto">
      ...
      <div className="bg-white rounded-[14px] border border-[#E2E8F0] overflow-hidden shadow-xs">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-[#E2E8F0]">
            {collections.map((col) => (
              <tr key={col.id} className="hover:bg-[#F8F9FF]">
                <td className="p-4 font-mono font-semibold text-[#3E4947]">{col.id}</td>
                <td className="p-4 font-mono text-[#6E7977]">{col.invoiceId}</td>
                <td className="p-4 font-bold text-[#0B1C30]">{col.customerName}</td>
                <td className="p-4 font-bold text-[#0F766E]">₹{col.amount.toLocaleString("en-IN")}</td>
                <td className="p-4 text-[#3E4947]">{col.date}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export const OutstandingView: React.FC = () => {
  const invoices = useDashboardStore((s) => s.invoices).filter((i) => i.status === "Pending");
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-[#F8F9FF] border-b border-[#E2E8F0] text-[#3E4947] font-semibold uppercase tracking-wider text-xs">
            <tr>
              ...
              <th className="p-4">Priority Alert</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#E2E8F0]">
            {invoices.map((inv) => (
              <tr key={inv.id} className="hover:bg-[#F8F9FF]">
                <td className="p-4 font-mono font-semibold text-[#3E4947]">{inv.id}</td>
                <td className="p-4 font-bold text-[#0B1C30]">{inv.customerName}</td>
                <td className="p-4 text-[#3E4947]">{inv.salesmanName}</td>
                <td className="p-4 font-bold text-[#BA1A1A]">₹{inv.amount.toLocaleString("en-IN")}</td>
                <td className="p-4 text-[#6E7977]">{inv.dueDate}</td>
                <td className="p-4">
                  <span className="px-2.5 py-1 rounded text-xs font-bold bg-[#FFDAD6] text-[#BA1A1A]">
                    High Priority
                  </span>
                </td>
              </tr>
